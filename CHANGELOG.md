...
- New `--no-config` flag to skip generating config utilities (`src/core/config.ts`, `tests/core/config.test.ts`, `benchmarks/config.bench.ts`)
- `includeConfig` option in `InitCommandOptions` and `TemplateContext` for conditional template rendering
- `FeatureFlag` type and `conditional` metadata on `TemplateModule` for declarative feature-gated templates

### Changed

- `src/mod.ts` template conditionally exports config utilities only when `includeConfig` is true
- `deno.jsonc` template conditionally includes `test` and `bench` tasks only when `includeConfig` is true
- Refactored template loading: removed unnecessary `template-loader.ts` indirection

## [0.2.13] - 2026-04-01

### Changed

- Added `name` and `version` fields to `deno.jsonc` template for immediate package identification
